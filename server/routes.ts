...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/admin/seo-stats", async (req, res) => {
    try {
      ...
      const allCities = ['caracas', 'maracaibo', 'valencia', 'barquisimeto', 'maracay', 'puerto-cabello', 'punto-fijo', 'ciudad-guayana', 'maturin', 'barcelona'];
      const allSectors = ['petrolero', 'petroquimico', 'siderurgico', 'minero', 'construccion', 'energia', 'industrial'];

      // Encontrar ciudades con poca cobertura
      const cityCoverage = allCities.map(city => {
      ...
      const sectorCoverage = allSectors.map(sector => {
      ...
      if (lowCoverageSectors.length > 0) {
        ...
        recommendations.push({
          ...
          details: { sector: topSectors[0]?.sector, count: topSectors[0]?.count },
        });
      }
      ...
      if (overusedCombinations.length > 0) {
        ...
        recommendations.push({
          ...
          details: { city: worstCombo.city || '', sector: worstCombo.sector || '', count: worstCombo.count },
        });
      }
      ...
      if (actualYesterday === 0) {
        recommendations.push({
          ...
          description: 'No se generaron blogs ayer. Revisa los logs del Scheduled Deployment y verifica que el cron esté configurado correctamente.',
        });
      }
      ...
      if (maxTemplate > minTemplate * 3 && byTemplate.length > 1) {
        const dominantTemplate = byTemplate.find(t => t.count === maxTemplate);
      ...
      if (recommendations.length === 0 && systemStatus === 'healthy') {
        recommendations.push({
          ...
          description: 'No hay acciones urgentes. Continúa monitoreando el rendimiento de las URLs y ajusta la estrategia según las vistas.',
        });
      }
