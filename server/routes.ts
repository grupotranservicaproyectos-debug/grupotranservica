...
import { insertContactRequestSchema, contactFormSchema, insertContactoRecibidoSchema, insertBlogSchema, blogs, blogGenerationLogs } from "@shared/schema";
import { z } from "zod";
  ...
  app.get("/api/blogs/health", async (req, res) => {
    try {
      ...
      const timeSinceLastGeneration = lastBlog?.publishedAt 
        ? Math.floor((now.getTime() - new Date(lastBlog.publishedAt).getTime()) / (1000 * 60 * 60))
        : null;

      const status = {
        healthy: blogsLast7Days[0]?.count > 0 && failedLogsLast7Days[0]?.count === 0,
