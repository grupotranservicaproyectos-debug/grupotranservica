...
export interface IStorage {
  ...
  updateBlogImages(id: string, coverImage: string, secondaryImages: string[]): Promise<Blog | undefined>;
...
export class MemStorage implements IStorage {
  ...
  async createBlog(insertBlog: InsertBlog): Promise<Blog> {
    ...
    const blog: Blog = {
      ...
      secondaryImages: insertBlog.secondaryImages ?? null,
      ogImage: insertBlog.ogImage ?? null,
      published: insertBlog.published ?? "true",
      autoGenerated: insertBlog.autoGenerated ?? "false",
      generationBatchId: insertBlog.generationBatchId ?? null,
      generationSource: insertBlog.generationSource ?? "manual",
      views: 0,
      createdAt: new Date(),
      updatedAt: new Date(),
  ...
  async updateBlogImages(id: string, coverImage: string, secondaryImages: string[]): Promise<Blog | undefined> {
    const blog = this.blogs.get(id);
    if (!blog) return undefined;
    const updated = { ...blog, coverImage, secondaryImages };
    this.blogs.set(id, updated);
    return updated;
  }
...
export class DBStorage implements IStorage {
  ...
  async updateBlogImages(id: string, coverImage: string, secondaryImages: string[]): Promise<Blog | undefined> {
    const result = await db.update(blogs)
      .set({ coverImage, secondaryImages })
      .where(eq(blogs.id, id))
      .returning();
    return result[0];
  }
